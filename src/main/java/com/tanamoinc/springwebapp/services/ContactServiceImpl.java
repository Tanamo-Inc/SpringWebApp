...
public class ContactServiceImpl extends BaseDAO implements ContactService {
    ...
    public List<Contact> findUserContact(Integer userId, String txt) {
        String sql = "SELECT _id, userId, cName, phone, email, address, remark FROM contact_table WHERE userId=? AND (cName LIKE '%" + txt + "%' OR address LIKE '%" + txt + "%' OR phone LIKE '%" + txt + "%' OR email LIKE '%" + txt + "%' OR remark LIKE '%" + txt + "%')";
